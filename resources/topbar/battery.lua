...
gears.timer {
	...
	local capacity_file = io.open(capacity_path, "rb") if capacity_file then
             charge = tonumber(capacity_file:read "*a")
             capacity_file:close()
	end
	local status_file = io.open(status_path, "rb") if status_file then
             local status = string.gsub(status_file:read "*a", "\n", "")
	     if status == "Full" then
                 charging = false
	     else
	         charging = true
